...
function advertisingIndex(data){
    ...
    for(var i in data){
        ...
        html += '<li style="background:url('+path+data[i].file+') no-repeat top; background-size:100% 100%;"><a class="banner_img"   href="javascript:void(0);"  onclick="updateAdsClickCount('+data[i].id+',\''+data[i].url+'\')" ></a>广告&nbsp;&nbsp;'+data[i].name+'</a></li>';
        updateAdsViewCount(data[i].id);
    }
    document.getElementById(advertisingElement).innerHTML = html;
    document.getElementById(advertisingElement).style.display = 'block';
